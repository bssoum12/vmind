...
export const JobTitleMultiSelect: React.FC<JobTitleMultiSelectProps> = ({
  ...
  const filteredJobTitles = useMemo(() => {
    if (!searchTerm.trim()) return JOB_TITLES;
    const normSearch = normalize(searchTerm);
    return JOB_TITLES.filter((title) => normalize(title).includes(normSearch));
  }, [searchTerm]);
  ...
  const toggleJobTitle = (title: string, e?: React.MouseEvent) => {
    ...
    if (selectedJobTitles.includes(title)) {
      ...
      onChange([...selectedJobTitles, title]);
    }
  };
    ...
    >
      ...
      >
        ...
        <input
          ...
          placeholder={selectedJobTitles.length === 0 ? placeholder : 'Ajouter un poste...'}
          style={{
            flex: 1,
            minWidth: '140px',
        ...
        >
          ...
          >
            {filteredJobTitles.length > 0 ? (
              ...
              >
                Aucun poste ne correspond à « {searchTerm} »
              </div>
            )}
          </div>
        </div>
