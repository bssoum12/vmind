...
export const IndustryMultiSelect: React.FC<IndustryMultiSelectProps> = ({
  ...
  const removeIndustry = (industry: string, e: React.MouseEvent) => {
    e.stopPropagation();
    onChange(selectedIndustries.filter((item) => item !== industry));
  };
            ...
            <span>
              {selectedIndustries.length > 0 ? (
                <strong style={{ color: 'var(--cyan, #00E5C8)' }}>
                  {selectedIndustries.length} secteur{selectedIndustries.length > 1 ? 's' : ''} sélectionné{selectedIndustries.length > 1 ? 's' : ''}
                </strong>
              ) : (
            ...
            {filteredIndustries.length > 0 ? (
              filteredIndustries.map((ind) => {
                ...
                return (
                  <div
                    key={ind}
                    onClick={(e) => toggleIndustry(ind, e)}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '0.65rem',
