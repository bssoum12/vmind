...
export const IntegrationsView: React.FC = () => {
  const integrations = [
    { name: 'TraLIS', category: 'ERP', status: 'Connected', icon: '📦' },
    { name: 'Sage 100', category: 'ERP', status: 'Available', icon: '🌿' },
    { name: 'Odoo', category: 'ERP', status: 'Available', icon: '🟣' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px', padding: '20px' }}>
          {integrations.map((int, i) => (
            <div key={i} className="wcard" style={{ display: 'flex', alignItems: 'center', gap: '16px', background: 'var(--navy3)' }}>
              <div style={{ fontSize: '24px', width: '48px', height: '48px', background: 'var(--navy4)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{int.icon}</div>
              <div style={{ flex: 1 }}>
